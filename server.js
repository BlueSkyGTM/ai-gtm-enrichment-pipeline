/**
 * FLEET ENGINE — AI GTM Enrichment Pipeline
 * ─────────────────────────────────────────────────────────────────
 * Architecture: Single-file orchestration of three specialized agents.
 *
 * Thesis: This pipeline was stress-tested to 1,500 enrichment cycles.
 * The agents worked. The infrastructure didn't. The original build ran
 * on self-hosted Postgres + n8n + Docker — three layers of hostile
 * environment pretending to be one coherent shared space. Every failure
 * was environmental, not architectural.
 *
 * This file is the proven agent layer, extracted from that environment.
 * The next iteration replaces the execution environment entirely —
 * native MCP tooling and Cloud-managed infrastructure instead of
 * invented shared state through open-source middleware.
 *
 * Agents:
 *   Ahab    — High-volume discovery. Scrapes, filters, seeds Postgres.
 *   Nemo    — Single-lead diagnostic enrichment. Produces friction profiles.
 *   Neptune — Outreach synthesis. Converts friction profiles into Bites.
 *
...
// ─── ROUTE: SEED (manual company intake) ─────────────────────────
...
app.post('/api/seed', async (req, res) => {
  ...
  try {
    ...
    res.json({ session_ids: raw_ids.filter(Boolean) });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
app.post('/api/nemo', async (req, res) => {
  ...
  const leadRes = await pool.query(
    'SELECT ahab_payload, company_name FROM gtm_career_leads WHERE session_id = $1 LIMIT 1',
    [input_session_id]
  );
  if (leadRes.rows.length === 0) return res.status(404).json({ error: 'Lead not found' });

  const ahab_payload = leadRes.rows[0].ahab_payload;
  if (!ahab_payload || Object.keys(ahab_payload).length === 0) {
    return res.status(400).json({ error: 'empty ahab_payload' });
  }
  ...
  try {
    ...
    if (result?.Nemo_Enrich_Audit?.status === 'SHIPWRECKED') {
      ...
      await pool.query(
        'INSERT INTO fleet_errors (session_id, reason_code, company_name) VALUES ($1, $2, $3)',
        [session_id, reason_code, leadRes.rows[0].company_name || null]
      );
...
async function runNeptuneSynthesis(input_session_id) {
  const leadRes = await pool.query(
    'SELECT nemo_payload, company_name FROM gtm_career_leads WHERE session_id = $1 LIMIT 1',
    [input_session_id]
  );
...
app.post('/api/requeue', async (req, res) => {
  try {
    ...
    res.json({ session_ids: result.rows.map(r => r.session_id) });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
app.listen(PORT, () => {
  ...
  console.log(`📡 Agents: Ahab → Nemo → Neptune`);
